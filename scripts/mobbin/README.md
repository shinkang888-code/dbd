...
## 0) 목록 검색 브릿지 (대시보드 검색창)

대시보드 `/studio/mobbin` 상단 **Mobbin 목록 검색**은 **로컬 브릿지**가 켜져 있을 때만
Mobbin을 파싱합니다. Vercel은 Mobbin에 직접 접속하지 않습니다.

```bash
npm i -D playwright
npx playwright install chromium
npm run mobbin:login          # Chrome에서 로그인 후 Enter
npm run mobbin:bridge         # http://127.0.0.1:3921
```

그 상태에서 대시보드(로컬 `localhost:3001` 또는 https://dbd-beta.vercel.app)를 열고
검색 → 목록 선택 → **이미지 가져오기** (프론트 1장, JPEG 저용량, 디스크 미저장).

이미 로그인된 Chrome에 붙이려면:

```bash
# Chrome을 디버그 포트로 실행한 뒤
set MOBBIN_CDP_URL=http://127.0.0.1:9222
